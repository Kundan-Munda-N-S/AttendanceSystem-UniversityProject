...
def AddName():
    Name = input('Enter the NAME :')    #raw_input('Enter Your Name ')
    Info = open("Names.txt", "r+")
    ID = input("Enter the ROLL_NUMBER : ")                                                                                                          #((sum(1 for line in Info))+1)
...
def DispID(x, y, w, h, NAME, Image):
 ...
 #  ------------------------------------    THE DRAWING OF THE BOX AND ID   --------------------------------------
    ...
    cv2.putText(Image, NAME, (Name_X_pos, Name_y_pos - 10), cv2.FONT_HERSHEY_DUPLEX, 2, WHITE,thickness = 2)                         # Print the name of the ID
